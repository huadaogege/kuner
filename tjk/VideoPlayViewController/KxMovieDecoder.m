...
#ifdef _USE_VT_
#import "VideoToolboxFFMPEG.h"
#endif

////////////////////////////////////////////////////////////////////////////////

static int static_b_useSDLaudio = 0;

static float static_cd_v_a = 0.6;


void Set_MAX_AV_CD(float cd);

void SetSdlFlag(char *pFileName)
{
    int len =strlen(pFileName);
    int pos=-1;
    if (len<=5)//肯定不是rmvb
    {
        static_b_useSDLaudio=0;
    }
    else
    {
        for (int i=0;i<len;i++)
        {
            if (pFileName[len-1-i] == '.') {
                pos =  len-i;
                break;
            }
        ...
        if(pos>=0)
        {
            if (strstr(pFileName+pos, "rmvb")!=NULL||strstr(pFileName+pos, "RMVB")!=NULL)
            {
                static_b_useSDLaudio=1;
            }
            else{
                static_b_useSDLaudio=0;
            }
...
int getIsNeedLongCDs()
{
    return static_b_useSDLaudio;
}
float Get_Max_CD()
{
    return static_cd_v_a;
}
...
- (void) setPosition: (CGFloat)seconds
{
    ...
    if(!_formatCtx || _dealloced) return;
    av_seek_frame(_formatCtx, -1, seconds * AV_TIME_BASE, AVSEEK_FLAG_BACKWARD);
    if(_formatCtx && !_dealloced){
...
- (kxMovieError) openInput: (NSString *) path
{
    ...
    SetSdlFlag([path cStringUsingEncoding: NSUTF8StringEncoding]);
    if (getIsNeedLongCDs()) {
        Set_MAX_AV_CD(0.7);
    }
    else
...
#ifdef _USE_VT_
    // Codec init to use videotoolbox h/w accelerate
    codecVideoToolboxInit(codecCtx);
#endif
...
-(void)checkisHandVideo
{
    if (isHandlerVideoFrame) {
        [NSThread sleepForTimeInterval:0.05];
        [self checkisHandVideo];
    }
    else if (isDecodeFrames) {
        [NSThread sleepForTimeInterval:0.05];
        [self checkisHandVideo];
    }
}
...
-(void)setDecodeFrameing:(BOOL)isdecodeFrame
{
    isDecodeFrames = isdecodeFrame;
}
...
- (NSArray *) decodeFrames: (CGFloat) minDuration
{
    ...
    while (!finished) {
        if(_formatCtx == NULL || self.isPlaying == NO){
            return result;
        }
...
//        AVPacketList * List = _formatCtx->packet_buffer;
//        int ss = _formatCtx->offset;
//        AVRational rr = _formatCtx->offset_timebase;

//        AVStream * av = _formatCtx->streams[_videoStream];
//        int test = av->skip_to_keyframe;
...
//
//                        NSLog(@"关键帧:%f = %f", (CGFloat)_formatCtx->duration, (CGFloat)_formatCtx->duration / AV_TIME_BASE);
//                        i= 0;
//                    }
//                    else{
//                        
...
#ifdef _USE_VT_
                    if (_videoFrame->format == AV_PIX_FMT_VIDEOTOOLBOX_VLD) {
//                        int err = -1;
//                        err = videotoolbox_retrieve_data(_videoCodecCtx, _videoFrame);
//                        if (err < 0) {
//                            break;
//                        }
                        NSLog(@"get videoFrame in format of AV_PIX_FMT_VIDEOTOOLBOX_VLD");
                    }
#endif
                    
                    if (!_disableDeinterlacing && _videoFrame &&
